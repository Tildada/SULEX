...
class ResBlock(nn.Module):
    def __init__(self, input_dim):
        ...
        self.ffn = FeedForward(input_dim=input_dim,
                               num_layers=1,
...
class BertModel_pos(BertModel):
    def __init__(self, config):
        super().__init__(config)

    def _get_position_embedding(self):
        return self.embeddings.position_embeddings.weight  ### 512, 768

    def _position_embedding_stop_grad(self):
        self.embeddings.position_embeddings.weight.requires_grad = False




class RobertaModel_pos(RobertaModel):
    def __init__(self, config):
        super().__init__(config)
...
class SpanEmbedEntity(BertPreTrainedModel):
    def __init__(self, config, num_ner_labels, head_hidden_dim=150, width_embedding_dim=150, max_span_length=8):
        ...
        self.init_weights()
        self.bert._position_embedding_stop_grad()
    ...
    def _get_span_embeddings(self, input_ids, spans, token_type_ids=None, attention_mask=None):
        ...
        spans_start_embedding = self.head_ffn(spans_start_embedding)  ## 768

        pos_embedding = self.bert._get_position_embedding()  ## 768
        pos_embedding = pos_embedding.unsqueeze(0).repeat(spans.size(0), 1, 1)
        pos_embedding = batched_index_select(pos_embedding, spans_start)
        ...
        spans_end_embedding = self.tail_ffn(spans_end_embedding)

        pooled_embedding = pooled_output.unsqueeze(1).repeat(1, spans_end_embedding.shape[1], 1)

        # Concatenate
        spans_embedding = torch.cat(
            (spans_start_embedding, pos_embedding, spans_width_embedding, spans_end_embedding, pooled_embedding),
            dim=-1)
        """
        spans_embedding: (batch_size, num_spans, hidden_size*4+embedding_dim)
        """
    ...
    def forward(self, input_ids, spans, spans_mask, spans_ner_label=None, token_type_ids=None, attention_mask=None,
                args=None, tot_step=None, cur_step=None, tot_epoch=None, cur_epoch=None, use_mask=False):
        ...
        logits = ffnn_hidden[-1]  # [bs, num_spans, ner_label_num]
        ...
        if spans_ner_label is not None:
            if args.loss_type == "ce":
                ...
                loss_function = nn.NLLLoss(ignore_index=-100, reduction='none')
                active_sigmoid = nn.Sigmoid()
                score = F.log_softmax(logits.view(-1, logits.shape[-1]), -1)
                active_loss = spans_mask.view(-1) == 1
                active_labels = torch.where(
                    active_loss, spans_ner_label.view(-1),
                    torch.tensor(loss_function.ignore_index).type_as(spans_ner_label)
                )
                ce_loss = loss_function(score, active_labels)

                if args.loss_type == "sig_ce_mask":
                    entity_mask = torch.tensor(spans_ner_label.view(-1) != 0, dtype=torch.int64) & active_loss
                    non_entity_mask = torch.tensor(spans_ner_label.view(-1) == 0, dtype=torch.int64) & active_loss
                    weight_score = active_sigmoid(logits.view(-1, logits.shape[-1]))
                    sig_ce_loss = loss_function(weight_score - 1,
                                                active_labels) * non_entity_mask + ce_loss * entity_mask
                    #             weight_score = active_sigmoid(logits.view(-1, logits.shape[-1]))
                    #             sig_ce_loss = loss_function(weight_score-1, active_labels)
                    if cur_epoch == None and tot_epoch == None:
                        loss = torch.mean(ce_loss)
                    else:
                        weight = math.exp(float(- cur_epoch) / 5)
                        # weight = 1-float(cur_epoch/tot_epoch)
                        loss = weight * torch.mean(ce_loss) + \
                               (1 - weight) * torch.mean(sig_ce_loss)
                else:  # args.loss_type == "sig_ce"
                    weight_score = active_sigmoid(logits.view(-1, logits.shape[-1]))
                    sig_ce_loss = loss_function(weight_score - 1, active_labels)
                    weight = math.exp(float(- cur_epoch) / 2)
                    loss = weight * torch.mean(ce_loss) + \
                           (1 - weight) * torch.mean(sig_ce_loss) * 2
...
class SpanEmbedRobertaEntity(RobertaPreTrainedModel):
    def __init__(self, config, num_ner_labels, head_hidden_dim=150, width_embedding_dim=150, max_span_length=8):
        super().__init__(config)
        ...
        self.init_weights()
        self.roberta._position_embedding_stop_grad()

    def _get_span_embeddings(self, input_ids, spans, token_type_ids=None, attention_mask=None):
        sequence_output, pooled_output = self.roberta(input_ids=input_ids, token_type_ids=token_type_ids,
                                                   attention_mask=attention_mask)
        sequence_output = self.hidden_dropout(sequence_output)
        ...
        pos_embedding = self.roberta._get_position_embedding()  ## 768
        pos_embedding = pos_embedding.unsqueeze(0).repeat(spans.size(0), 1, 1)
        pos_embedding = batched_index_select(pos_embedding, spans_start)
...
class SpanEmbedEntityModel():
    ...
    def __init__(self, args, num_ner_labels):
        ...
        if args.use_roberta:
            ...
            self.bert_model = SpanEmbedRobertaEntity.from_pretrained(bert_model_name, num_ner_labels=num_ner_labels,
                                                              max_span_length=args.max_span_length, return_dict=False)
        else:
            self.tokenizer = BertTokenizer.from_pretrained(vocab_name)
            self.bert_model = SpanEmbedEntity.from_pretrained(bert_model_name, num_ner_labels=num_ner_labels,
                                                            max_span_length=args.max_span_length, return_dict=False)
    ...
    def run_batch(self, samples_list, try_cuda=True, training=True, tot_step=None, cur_step=None, tot_epoch=None,
                  cur_epoch=None):
