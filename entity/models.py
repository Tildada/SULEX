...
class myEntity(BertPreTrainedModel):
            ...
            if args.loss_type == "ce":
                ...
                if args.loss_type == "sig_ce_mask":
                    ...
                    if cur_epoch == None and tot_epoch == None:
                        ...
                        weight = math.exp(float(- cur_epoch) / args.w1)  # w1 = 5
                        # weight = 1-float(cur_epoch/tot_epoch)
                        loss = weight * torch.mean(ce_loss) + (1 - weight) * torch.mean(sig_ce_loss) * args.w2  # w2 = 1

                elif args.loss_type == "sig_ce":
                    weight_score = active_sigmoid(logits.view(-1, logits.shape[-1]))
                    sig_ce_loss = loss_function(weight_score - 1, active_labels)
                    weight = math.exp(float(- cur_epoch) / args.w1)  # w1 = 2
                    loss = weight * torch.mean(ce_loss) + \
                           (1 - weight) * torch.mean(sig_ce_loss) * args.w2  # w2 = 1

                elif args.loss_type == "ab1":
                    sig_score = active_sigmoid(logits.view(-1, logits.shape[-1]))
                    sig_score = torch.mean(sig_score, dim=-1)
                    weight = math.exp(float(- cur_epoch) / args.w1)  # w1 = 5
                    sig_ce_loss = weight * ce_loss + (1 - weight) * sig_score
                    entity_mask = torch.tensor(spans_ner_label.view(-1) != 0, dtype=torch.int64) & active_loss
                    non_entity_mask = torch.tensor(spans_ner_label.view(-1) == 0, dtype=torch.int64) & active_loss
                    loss = torch.mean(ce_loss) + torch.mean(non_entity_mask * sig_ce_loss)

                elif args.loss_type == "ab2":
                    sig_score = active_sigmoid(logits.view(-1, logits.shape[-1]))
                    sig_score = torch.mean(sig_score, dim=-1)
                    weight = math.exp(float(- cur_epoch) / args.w1)  # w1 = 5
                    sig_ce_loss = weight * ce_loss + (1 - weight) * sig_score
                    entity_mask = torch.tensor(spans_ner_label.view(-1) != 0, dtype=torch.int64) & active_loss
                    non_entity_mask = torch.tensor(spans_ner_label.view(-1) == 0, dtype=torch.int64) & active_loss
                    loss = torch.mean(entity_mask * ce_loss) + torch.mean(sig_ce_loss)
...
class myRobertaEntity(RobertaPreTrainedModel):
    def __init__(self, config, num_ner_labels, head_hidden_dim=150, width_embedding_dim=150, max_span_length=8):
        super().__init__(config)
    ...
    def forward(self, input_ids, spans, spans_mask, spans_ner_label=None, token_type_ids=None, attention_mask=None,
        ...
        if spans_ner_label is not None:
            ...
            active_loss = spans_mask.view(-1) == 1
            active_labels = torch.where(
                active_loss, spans_ner_label.view(-1), torch.tensor(loss_function.ignore_index).type_as(spans_ner_label)
            )
            ce_loss = loss_function(score, active_labels)
            ...
            non_entity_mask = torch.tensor(spans_ner_label.view(-1) == 0, dtype=torch.int64) & active_loss
            weight_score = active_sigmoid(logits.view(-1, logits.shape[-1]))
            sig_ce_loss = loss_function(weight_score - 1, active_labels) * non_entity_mask + ce_loss * entity_mask
            #             weight_score = active_sigmoid(logits.view(-1, logits.shape[-1]))
            #             sig_ce_loss = loss_function(weight_score-1, active_labels)
            if cur_epoch == None and tot_epoch == None:
                loss = torch.mean(ce_loss)
...
class EntityModel():
    ...
    def __init__(self, args, num_ner_labels):
        ...
        if args.use_roberta:
            ...
            self.bert_model = myRobertaEntity.from_pretrained(bert_model_name, num_ner_labels=num_ner_labels,
                                                       max_span_length=args.max_span_length, return_dict=False)
        else:
            self.tokenizer = BertTokenizer.from_pretrained(vocab_name)
            self.bert_model = myEntity.from_pretrained(bert_model_name, num_ner_labels=num_ner_labels,
                                                       max_span_length=args.max_span_length, return_dict=False)
