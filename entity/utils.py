...
def batchify(samples, batch_size):
    ...
    for i in range(0, len(samples)):
        if "tokens" not in samples[i].keys():
            if len(samples[i]['input_ids']) > 350:  # msra
...
#             list_samples_batches.append([samples[i]])
#         else:
#             not_single_batch.append(samples[i])
...
#     for i in range(0, len(not_single_batch), batch_size):
#         list_samples_batches.append(not_single_batch[i:i + batch_size])

#     #     assert(sum([len(batch) for batch in list_samples_batches]) == num_samples)

#     sample_shuffle = []
#     index = list(range(len(list_samples_batches)))
#     random.shuffle(index)
#     for i in index:
#         sample_shuffle.append(list_samples_batches[i])

#     return sample_shuffle
...
def convert_dataset_to_samples_drop_ents_neg_sample(dataset, max_span_length, drop_p = 0.8, ner_label2id=None, split=0, neg_sample = 0.5):
    """
    Extract sentences and gold entities from a dataset
    """
    # split: split the data into train and dev (for ACE04)
    ...
    for c, doc in enumerate(dataset):
        ...
        for i, sent in enumerate(doc):
            ...
            sample['spans'] = []
            sample['spans_label'] = []
            rand_matrix = np.random.random((len(sent.text), len(sent.text)))
            for i in range(len(sent.text)):
                for j in range(i, min(len(sent.text), i+max_span_length)):
                    if (i, j) not in sent_ner:
                        ...
                        if np.random.rand() < drop_p:
                            sample['spans'].append((i + sent_start, j + sent_start, j - i + 1))
                            sample['spans_label'].append(ner_label2id[sent_ner[(i, j)]])
                        elif np.random.rand() < neg_sample:
                            sample['spans'].append((i + sent_start, j + sent_start, j - i + 1))
                            sample['spans_label'].append(0)
                            drop_ner += 1
            if sample['spans'] != []:
    ...
    logger.info('Max Length: %d, max NER: %d'%(max_len, max_ner))
    logger.info('Drop NER num: %d'%(drop_ner))
    return samples, num_ner


def convert_dataset_to_samples_drop_ents(dataset, max_span_length, drop_p=0.8, ner_label2id=None, context_window=0,
                                         split=0):
    """
    Extract sentences and gold entities from a dataset
    """
